...
def test_file_structure(downloader):
    ...
    num_downloaded = dl.get(filing_type, ticker, 1, include_filing_details=False)
    ...
    filing_type = "10-K"
    num_downloaded = dl.get(filing_type, ticker, 1, include_filing_details=True)
    assert num_downloaded == 1
...
def test_file_contents(downloader):
    ...
    num_downloaded = dl.get(
        filing_type, ticker, 1, before_date=before_date, include_filing_details=False
    )
    assert num_downloaded == 1
